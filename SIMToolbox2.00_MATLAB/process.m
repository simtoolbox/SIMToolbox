...
btn = questdlg([sprintf('Output images were saved into the data directory:\n') ...
    cfg.datadir], 'Processing was successfully finished.', ...
    'Ok','Open folder','Ok');
...
%------------------------------------------
names = fieldnames(im);
I = find([cfg.save.enable]);
for m = 1:length(names)
    feval(cfg.save(I).fnc,imginfo.camera.norm*im.(names{m}), ...
          [cfg.resdir imginfo.data.filemask '_' names{m} '.tif']);
end
