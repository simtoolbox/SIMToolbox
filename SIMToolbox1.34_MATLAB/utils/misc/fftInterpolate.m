...
% out = assignSubRange(out,rng,inft);
out = ifftshift(fac*ifftn(ifftshift(assignSubRange(out,rng,inft))));
...
for d = 1:dim
    if lo(d) == 0, lo(d) = 1; end
    if hi(d) == 0, hi(d) = sz(d); end
end
...
switch dim
    ...
    otherwise
        ...
        str = 'subData = data(';
        for d = 1:dim
            str = [str 'lo(' num2str(d) '):hi(' num2str(d) ')'];
            if (d~=dim)
                str = [str ','];
            else
                str = [str ');'];
