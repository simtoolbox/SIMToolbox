...
hmain = figure('Name',sprintf('SIM Toolbox (ver. %s)',cfg.ver),'Tag','dlgMain','Resize','off',...
    'Units','pixels','Position',[(psiz(3)-2*mainwidth)/2,(psiz(4)-mainheight)/2,2*mainwidth,mainheight],...
...
hlogo = axes('Parent',hmain,...
    'Tag','axesLogo',...
    'Units','pixels','Position',[(mainwidth-imWidth)/2,(psiz(2)-imHeight-20),imWidth,imHeight]);
...
hdata = uipanel('Parent',hmain,'Units','pixels',...
    'Position',[dlgmargin,psiz(2)-height,mainwidth-2*dlgmargin,height],...
    'Title','Data');
...
uicontrol('Parent',hptrn,'Style','pushbutton','String','Run calibration',...
    ...
    'Units','pixels','Position',[mainwidth-4*dlgmargin-100,top-hLine,100,hBtn]);
...
uicontrol('Parent',hptre,'Style','pushbutton','String','Find peaks',...
    ...
    'Units','pixels','Position',[mainwidth-4*dlgmargin-100,top-1*hLine,100,hBtn]);

% ----------- PROGRESS BAR --------------------
axes('Parent',hmain,...
    'Tag','axPrgBar',...
    'Units','Pixels','Position',[dlgmargin,psiz(2)-dlgmargin-hProg,mainwidth-2*dlgmargin,hProg],...
...
hsim = uipanel('Parent',hmain,'Units','pixels',...
    'Position',[mainwidth+dlgmargin,psiz(2)-height-10,mainwidth-2*dlgmargin,height],...
...
hmsm = uipanel('Parent',hmain,'Units','pixels',...
    'Position',[mainwidth+dlgmargin,psiz(2)-height,mainwidth-2*dlgmargin,height],...
...
uicontrol('Parent',hmsm,'Style','text','String',...
    ...
    'Units','pixels','Position',[mainwidth-4*dlgmargin-150,top-3*hLine-3,150,4*hTxt],...
...
uicontrol('Parent',hmsm,'Style','text','String','',...
    'Units','pixels','Position',[dlgmargin,top-6.3*hLine,mainwidth-4*dlgmargin,3*hTxt],...
...
uicontrol('Parent',hmain,'Style','pushbutton','String','Reset settings',...
    ...
    'Units','pixels','Position',[mainwidth+dlgmargin,psiz(2)-3*dlgmargin-hBtn,90,hBtn]);

% Preview button
uicontrol('Parent',hmain,'Style','pushbutton','String','Preview',...
    'Tag','btnPreview',...
    'Callback','SIMToolbox(''btnPreview_Callback'',gcbo,[])',...
    'Units','pixels','Position',[mainwidth+dlgmargin+100,psiz(2)-3*dlgmargin-hBtn,90,hBtn]);

% Preview - z
uicontrol('Parent',hmain,'Style','text','String','z:',...
    'Units','pixels','Position',[mainwidth+dlgmargin+200,1+psiz(2)-3*dlgmargin-hBtn,12,hTxt],...
    'HorizontalAlignment','right');

uicontrol('Parent',hmain,'Style','edit',...
    'Tag','editPreviewZ',...
    'Callback','SIMToolbox(''editPreview_Callback'',gcbo,''z'')',...
    'Units','pixels','Position',[mainwidth+dlgmargin+212,psiz(2)-3*dlgmargin-hBtn,30,hEdtBx],...
    'BackgroundColor','w');

% Preview - time
uicontrol('Parent',hmain,'Style','text','String','t:',...
    'Units','pixels','Position',[mainwidth+dlgmargin+250,1+psiz(2)-3*dlgmargin-hBtn,12,hTxt],...
    'HorizontalAlignment','right');

uicontrol('Parent',hmain,'Style','edit',...
    'Tag','editPreviewTime',...
    'Callback','SIMToolbox(''editPreview_Callback'',gcbo,''t'')',...
    'Units','pixels','Position',[mainwidth+dlgmargin+262,psiz(2)-3*dlgmargin-hBtn,30,hEdtBx],...
    'BackgroundColor','w');
...
uicontrol('Parent',hmain,'Style','pushbutton','String','Run',...
    ...
    'Units','pixels','Position',[2*mainwidth-dlgmargin-150,psiz(2)-3*dlgmargin-hBtn,80,hBtn]);

% Quit button
uicontrol('Parent',hmain,'Style','pushbutton','String','Quit',...
    'Tag','btnQuit',...
    'Callback','SIMToolbox(''btnQuit_Callback'',gcbo,[])',...
    'Units','pixels','Position',[2*mainwidth-dlgmargin-60,psiz(2)-3*dlgmargin-hBtn,60,hBtn]);
...
switch h.String
    ...
    case 'Preview'
        ...
        updateVsmPreview((0:os)./p);
        updateSimPreview([os,os+sr]./p);
        updateMsmPreview([os+sr,os+sr+mp]./p);
...
if data.cfg.sim.enable && any(angles)
    ...
    progressbarGUI(data.hndl.axPrgBar,se(6),'Creating SR-SIM preview ...');
    data.seq = sim_addOTF(data.seq,data.ptrn,data.cfg.sim.otf);
    % combine spectra together
    progressbarGUI(data.hndl.axPrgBar,se(7),'Creating SR-SIM preview ...');
    ...
    progressbarGUI(data.hndl.axPrgBar,se(8),'Creating SR-SIM preview ...');
    % show figures
...
if any(angles)
    ...
    if strcmp(data.cfg.msm.meth,'CUDA')
        MapcoreCudaPrepare([data.imginfo.image.size.x,data.imginfo.image.size.y,...
            sum([data.ptrn.enable])*data.imginfo.image.size.seq/length(data.ptrn),...
...
set(data.hndl.txtCudaInfo,'Enable',msim,'Visible',...
    strrep(strrep(sprintf('%d ',mapcuda),'1','on'),'0','off'));
