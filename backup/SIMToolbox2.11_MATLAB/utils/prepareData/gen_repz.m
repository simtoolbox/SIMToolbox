...
%         waitbar(J/length(ptrn.runningorder(I).data), cfg.hndlwb,'Creating patterns');
        progressbarGUI(cfg.hndlwb,0.5*J/length(ptrn.runningorder(I).data));
    end
...
progressbarGUI(cfg.hndlwb,'Saving patterns');
% save images
[foo,idx] = unique(imgnames);
pbidx = linspace(0,0.4,length(idx));
for J = 1:length(idx)
    imwrite(double(IM(:,:,idx(J))),[tmpdir filesep imgnames{idx(J)}]);
    progressbarGUI(cfg.hndlwb,0.5+pbidx(J),'Saving patterns');
end

% waitbar(1, cfg.hndlwb,'Generating repz file');
progressbarGUI(cfg.hndlwb,'Generating repz file');
...
% rename repz
movefile([cfg.ptrndir filesep fname '.zip'], [cfg.ptrndir filesep fname '.repz'],'f');
