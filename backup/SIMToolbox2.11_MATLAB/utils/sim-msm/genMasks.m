...
%     waitbar(itheta/(2*numangles), hndlwb, 'Generating illumination masks ...');
    nPhases = ptrn(itheta).num;
    if ~isempty(ptrn(itheta).phase)
        fi = ptrn(itheta).phase';
    else
        fi = 2*pi*(0:nPhases-1)/nPhases;
    end
    pattern = zeros(sy,sx,nPhases);
    ...
    for iphi = 1:nPhases
        ...
        temp = cos(2*pi*kx*xx+2*pi*ky*yy+fi(iphi));
%         temp(temp<0) = 0;
%         temp(temp>0) = 1;
        pattern(:,:,iphi) = (temp+1)./(2*nPhases);
        
        progressbarGUI(hndlwb,(iphi+(th-1)*nPhases)/(nPhases*nAngles));
    end
    th = th + 1;
    MaskOn(itheta).angle = ptrn(itheta).angle;
    MaskOn(itheta).num = nPhases;
