function  [peaks,angl] = sim_findpeaks(im,method,ptrn,calinfo,cfg,idx,ax)
...
switch method.type
    case 'calibration'
        ...
        if isfield(calinfo.cal.map,'Rw2i')
            ...
            fit = [];
            [angl,spac] = cart2pol(peaks(1).x,size(im,2)*peaks(1).y/size(im,1));
        end
    case 'spotfinder'
        % no calibration - find peaks in the data
        [period,peaks,fit,angl,spac] = sim_findpeaks_data(im,ptrn,method,idx);
    otherwise
        error('sim:findpeaks:method','Unknown method.');
end

if cfg.plotpeaks
    sim_findpeaks_draw(im,period,peaks,fit,ax);
    
    spac = 1e3*size(im,2)/spac*cfg.sim.otf.params.resolution;
...
%-----------------------------------------------------------------------------
function [period,pos,fit,angl,xrad] = sim_findpeaks_data(im,ptrn,method,idx)
%-----------------------------------------------------------------------------
...
%-----------------------------------------------------------------------------
function sim_findpeaks_draw(im,period,peaks,fit,ax)
%-----------------------------------------------------------------------------
