function  [peaks,angle] = sim_findpeaks(im, method, ptrn, calinfo, cfg)
...
switch method.type
  case 'calibration'
    ...
    if isfield(calinfo.cal.map,'Rw2i')
      ...
      [period, peaks] = sim_findpeaks_calhomolin(im, ptrn, calinfo); fit = []; angle=[];
    end
  case 'spotfinder'
    % no calibration - find peaks in the data
    [period, peaks, fit, angle] = sim_findpeaks_data(im, ptrn, method);
  otherwise
...
%-----------------------------------------------------------------------------
function [period, pos, fit,angle] = sim_findpeaks_data(im, ptrn, method)
%-----------------------------------------------------------------------------
...
clf;
imagesc(im,[-3 10]);
axis off equal; set(gca,'Position',[0 0 1 1]);
hold on
