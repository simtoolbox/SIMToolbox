function [A, params] = apodize_standard(siz, params)
% 2D standard incoherent apodizing filter
%
%   [A, params] = apodize_standard(siz, params)
%
...
% Filter definition:
%
%   A = 2/pi*(acos(v) - v.*sqrt(1-v.^2))  for radius <= params.rad
%   A = 0                                 otherwise
%
% Example:
%
%   A = apodize_standard([255 255], struct('rad',0.5));
...
% Standard incoherent OTF
v = r/rad; % normalized spatial frequency
A = 2/pi*(acos(v) - v.*sqrt(1-v.^2)) .* (r <= rad);
