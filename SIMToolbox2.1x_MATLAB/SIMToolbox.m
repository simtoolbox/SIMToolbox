...
set(h,'String',num2str(data.cfg.msm.fc,'%.2f'));
...
set(h,'String',num2str(data.cfg.msm.wmerg,'%.2f'));
...
if data.cfg.sim.enable && any(angles)
    ...
    progressbarGUI(data.hndl.axPrgBar,se(1),'Calculating SR-SIM preview ...');
    % shift expanded spectra to center position
    seq = sim_shiftspectra(seq,data.ptrn,data.cfg.sim);
    progressbarGUI(data.hndl.axPrgBar,se(2),'Calculating SR-SIM preview ...');
    % generating shifted OTFs
    seq = sim_addOTF(seq,data.ptrn,data.cfg.sim.otf);
    progressbarGUI(data.hndl.axPrgBar,se(3),'Calculating SR-SIM preview ...');
    % combine spectra together
    [IMsr,IMsrFFT] = sim_combine(seq(angles),data.cfg.sim);
    progressbarGUI(data.hndl.axPrgBar,se(4),'Calculating SR-SIM preview ...');
...
if any(angles)
    ...
    progressbarGUI(data.hndl.axPrgBar,se(1),'Calculating MAP-SIM preview ...','green');
