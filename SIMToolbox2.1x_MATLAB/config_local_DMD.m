...
cfg.datadir = 'data/';
cfg.resdir = ['$data$' filesep 'results' filesep];
...
cfg.sim.harmonweight = [0 1 0 0]; % weight on harmonics (first index is 0th harmonics)
cfg.sim.wiener = 0.1;
cfg.sim.otf = struct('type','standard','params',struct('rad',0.9));
cfg.sim.apodize = struct('type','cosbell','params',struct('rad',0.7));
cfg.sim.enable = 0;
cfg.sim.upsample = 0;
...
% MAP-SIM CUDA
cfg.msm.cuda.alph = 0.5;            % Initial Alpha
cfg.msm.cuda.maxiter = 5;           % Maximum number of iterations allowed
cfg.msm.cuda.lamb  = 0.0001;        % Lambda (Normalization Coefficient)
cfg.msm.cuda.thresh = 0.01;         % Convergence Threshold
...
cfg.db.spotfinder.radius = 28;         %  corresponds to spatial frequency of a 1st harmonics
