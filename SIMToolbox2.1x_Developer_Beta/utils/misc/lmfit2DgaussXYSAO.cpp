...
//
// [par, exitflag, residua, numiter, message] = lmfit2DgaussXYSAO(par0, X, y, options);
//
...
{
	return par[3] * exp( -(SQR(X[0]-par[0])+SQR(X[1]-par[1]))/SQR(par[2]) ) + par[4];
}
...
void partransf(double *parin, double *parout, size_t npar)
{
	...
	parout[2] = SQR(parin[2]);  // sigma >= 0
	parout[3] = SQR(parin[3]);  // amplitude >= 0
	parout[4] = SQR(parin[4]);  // offset >= 0
}
...
{ 
	...
	if (nrhs < 3) {
		mexPrintf("\nLevenberg-Marquardt Least Squares Fitting of 2D Gaussian kernel\n\n");		
        mexPrintf("   f = A * exp ( -((x-x0)^2 + (y-y0)^2) / sigma^2 ) + offset\n\n");
		mexPrintf(" Usage:\n");
		mexPrintf("  [par, reserr, exitflag, numiter, message] = lmfit2DgaussXYSAO(par0, X, y, options);\n\n");
		mexPrintf(" In:\n");
		mexPrintf("   par0     ... [1 x npar]      initial function parameters [x0, y0, sigma, amplitude, offset]\n");
		...
		mexPrintf(" Out:\n");
		mexPrintf("   par      ... [1 x npar]      optimized parameters [x0, y0, sigma, amplitude, offset]\n");
		mexPrintf("   reserr   ...                 sum of residual errors: sum((y-fun(X,par)).^2)\n");
	...
	// residual error 
	if (nlhs > 1)
		plhs[1] = mxCreateDoubleScalar(sumreserr(par, data.npts, &data));
