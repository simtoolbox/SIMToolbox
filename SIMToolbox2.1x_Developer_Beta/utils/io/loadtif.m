function stack = loadtif(fname,hndlwb)
...
progressbarGUI(hndlwb,0,'Loading stack ...');

for ii=1:Nframes
%    fig=waitbar(ii/Nframes,fig);
   progressbarGUI(hndlwb,ii/Nframes);
   TifLink.setDirectory(ii);
   stack(:,:,ii)=TifLink.read();
end
TifLink.close();
% delete(fig);
progressbarGUI(hndlwb,1,'Stack is successfully loaded');
